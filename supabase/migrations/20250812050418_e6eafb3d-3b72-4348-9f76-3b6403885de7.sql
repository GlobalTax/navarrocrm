...
-- Create table to log sent reminders and avoid duplicates
create table if not exists public.deed_reminder_logs (
  id uuid primary key default gen_random_uuid(),
  org_id uuid not null,
  deed_id uuid not null,
  reminder_type text not null check (reminder_type in ('modelo600','asiento','calificacion')),
  days_before integer not null,
  created_at timestamptz not null default now(),
  unique (org_id, deed_id, reminder_type, days_before)
);

-- Enable RLS
alter table public.deed_reminder_logs enable row level security;

-- Policies: allow org members to read their logs; allow inserts/updates by org members (service role bypasses anyway)
create policy if not exists "Org members can view their deed reminder logs"
...
create policy if not exists "Org members can insert deed reminder logs"
  on public.deed_reminder_logs
  for insert
  with check (exists (
    select 1 from public.users u
...
create policy if not exists "Org members can update their deed reminder logs"
  on public.deed_reminder_logs
  for update
  using (exists (
    select 1 from public.users u
...
-- Schedule daily invocation at 08:00 UTC (adjust if needed)
