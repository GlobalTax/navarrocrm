-- P0 • Empleados: Organigrama por departamentos y responsable
-- 1) Schema change: add manager_user_id to departments + index + RLS select policy (idempotent)
...
-- Create SELECT policy if not exists: users can view departments from their org
DO $$
BEGIN
  IF NOT EXISTS (
...
-- 2) Backfill manager_user_id based on users with role partner/area_manager and matching department
...
-- 3) View: v_department_org_chart with security_invoker
  ...
  SELECT 
    ...
    jsonb_agg(
      jsonb_build_object(
        ...
        'team_lead_name', COALESCE(ul.name, ul.email),
...
SELECT 
  ...
  d.manager_user_id,
  COALESCE(um.name, um.email) AS manager_name,
  um.avatar_url AS manager_avatar,
