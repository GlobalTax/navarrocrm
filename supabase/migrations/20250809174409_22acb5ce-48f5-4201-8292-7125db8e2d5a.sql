-- Fix view v_department_org_chart: use first_name/last_name instead of name
  ...
  SELECT 
    ...
    jsonb_agg(
      jsonb_build_object(
        ...
        'team_lead_name', COALESCE(NULLIF(TRIM(CONCAT(ul.first_name,' ',ul.last_name)), ''), ul.email),
...
SELECT 
  ...
  d.manager_user_id,
  COALESCE(NULLIF(TRIM(CONCAT(um.first_name,' ',um.last_name)), ''), um.email) AS manager_name,
